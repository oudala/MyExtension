class LinkDropExtension {
  ...
  setupEventListeners() {
    ...
    // Refresh links
    document.getElementById('refresh-links')?.addEventListener('click', () => this.loadRecentLinks());
  }
  ...
  async handleRegister(e) {
    ...
    try {
      ...
      this.showSuccess('Account created successfully! Welcome to LinkDrop! 🎉');
    } catch (error) {
  ...
  async handleShare() {
    ...
    try {
      ...
      this.updateShareButton(); // Re-enable based on selections
    }
  }
  ...
  async loadRecentLinks() {
    try {
      ...
      this.renderRecentLinks(response.links.slice(0, 5));
    } catch (error) {
  ...
  renderRecentLinks(links) {
    ...
    linksList.innerHTML = links.map(link => {
      ...
      return `
        <div class="link-item ${isUnread ? 'unread' : ''}" data-link-id="${link._id}" data-url="${link.url}">
  ...
  updateGroupsSelection() {
    ...
    if (!this.groups || this.groups.length === 0) {
      groupsList.innerHTML = '<p class="no-friends">No groups yet. Create or join groups in the dashboard!</p>';
      return;
    }
    ...
    groupsList.innerHTML = this.groups.map(group => {
      ...
      return `
        <div class="group-item ${isSelected ? 'selected' : ''}" data-group-id="${group._id}">
    ...
    groupsList.querySelectorAll('.group-item').forEach(item => {
      ...
      item.addEventListener('click', () => this.toggleGroupSelection(groupId));
    });
  }
  ...
  updateShareButton() {
    ...
    if (totalCount === 0) {
      shareBtn.disabled = true;
      shareBtn.querySelector('.button-text').textContent = 'Share Link';
    } else {
      shareBtn.disabled = false;
      let text = 'Share with ';
      if (friendCount > 0) {
      ...
      shareBtn.querySelector('.button-text').textContent = text;
    }
  }
